...
@NamedQueries({
	@NamedQuery(name="PrePaidPaymentEntity.getPropertyNum",query="select p.propertyId,p.property_No from Property p"),
	@NamedQuery(name="PrePaidPaymentEntity.getMeterNum",query="select pp.mId,pp.meterNumber,pp.propertyId from PrePaidMeters pp"),
	@NamedQuery(name="PrePaidPaymentEntity.getAll",query="select pa.adjustmentId,pa.consumerId,pa.propertyId,pa.adjustmentDate,pa.adjustmentNo,pa.adjustmentAmount,pa.adjustmentType,pa.status,pa.remarks,pa.instrumentDate,pa.instrumentNo,pa.bankName,pa.ca_No from PrePaidPaymentEntity pa ORDER BY pa.adjustmentDate DESC"),
	@NamedQuery(name="PrePaidPaymentEntity.getCaNumbers",query="select pm.ca_no from PrePaidMeters pm where pm.propertyId=:propertyId"),
	@NamedQuery(name="PrePaidPaymentEntity.searchByMonth",query="select pa.adjustmentId,pa.consumerId,pa.propertyId,pa.adjustmentDate,pa.adjustmentNo,pa.adjustmentAmount,pa.adjustmentType,pa.status,pa.remarks,pa.instrumentDate,pa.instrumentNo,pa.bankName,pa.ca_No from PrePaidPaymentEntity pa where TRUNC(pa.adjustmentDate) BETWEEN TO_DATE(:fromDate,'YYYY-MM-DD') AND TO_DATE(:toDate,'YYYY-MM-DD') ORDER BY pa.adjustmentDate DESC"),
