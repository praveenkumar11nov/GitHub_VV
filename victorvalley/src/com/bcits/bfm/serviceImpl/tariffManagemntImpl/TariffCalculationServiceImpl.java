...
/******************************************* Tariff Calculation (Chandra code ends)********************************************************************/
	...
	public String getCamPreviousBillstatusSpecific(Integer accountId) {
		...
		try {
			status=(String) entityManager.createQuery("SELECT elb.status FROM ElectricityBillEntity elb WHERE elb.typeOfService='CAM' AND elb.accountId='"+accountId+"' AND elb.status NOT IN ('Cancelled') ORDER BY elb.elBillId DESC").setMaxResults(1).getSingleResult();
...
logger.info(":::::::accountId::::::"+accountId+"::::::::::::"+map.get("DGInitialReading"));
	...
	if(blockId==722){
	String proList=
	"SELECT PROPERTY_ID,PROPERTY_NO FROM PROPERTY WHERE BLOCK_ID IN(SELECT BLOCK_ID FROM BLOCK WHERE BLOCK_NAME LIKE 'Tower-D%')ORDER BY PROPERTY_NO";
	List<?> DblockProList = entityManager.createNativeQuery(proList).getResultList();
	for (Iterator iterator = DblockProList.iterator(); iterator.hasNext();) {
		Object[] pro = (Object[]) iterator.next();
		propertyNumberMap = new HashMap<String, Object>();		
		propertyNumberMap.put("propertyId",Integer.parseInt(pro[0].toString()));
		propertyNumberMap.put("property_No",pro[1].toString());
		result.add(propertyNumberMap);
	}
	return result;
	}
 ...
 return result;
	...
	@Override
	public int getCountOfBillForCurrentMonth(int month,int year,int accountId,Date curBillDate) throws ParseException{
		BigDecimal count =(BigDecimal) entityManager.createNativeQuery("SELECT count(*) FROM BILL WHERE EXTRACT(month FROM BILL_DATE)="+month+" AND "
		+ "EXTRACT(year FROM BILL_DATE)="+year+" AND STATUS NOT IN('Cancelled') AND ACCOUNT_ID='"+accountId+"'").getSingleResult();	
		
		List<?> check = entityManager.createNativeQuery("SELECT FROM_DATE,TO_DATE FROM BILL WHERE STATUS NOT IN('Cancelled') AND ACCOUNT_ID='"+accountId+"' ORDER BY elb_id DESC").getResultList();
		Date from,to = null;
		int num = 0;
		for (Iterator<?> iterator = check.iterator(); iterator.hasNext();){
			final Object[] values = (Object[]) iterator.next();
			from = new SimpleDateFormat("yyyy-MM-dd").parse(values[0].toString());
			to   = new SimpleDateFormat("yyyy-MM-dd").parse(values[1].toString());
			//CAMBillsController.writeToFile("curBillDate="+curBillDate+"   from="+from+"   to="+to);
			if(curBillDate.compareTo(from)>=0 && curBillDate.compareTo(to)<0){
				num++;
			}
		}
