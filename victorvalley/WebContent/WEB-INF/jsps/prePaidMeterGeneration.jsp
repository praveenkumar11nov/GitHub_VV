...
<kendo:grid name="grid"  pageable="false" resizable="true" change="onChangeBillsList" edit="prepaidMeterSettingEvent" sortable="false" reorderable="true" selectable="false" scrollable="true" filterable="false" groupable="false">
	...
	<kendo:grid-editable mode="popup"/>
		<kendo:grid-toolbar>
		    ...
		    <%--   <kendo:grid-toolbarItem name="webService" text="Call Web Service"></kendo:grid-toolbarItem> --%>
	...
	<kendo:grid-columns>
	    ...
	    <kendo:grid-column title="Tower&nbsp;*" field="blockId" width="70px" filterable="false" hidden="true"/>
	     ...
	     <kendo:grid-column title="CA&nbsp;Number" field="ca_no"  filterable="true" 
	   ...
	   <%--  <kendo:grid-column-filterable>
	     ...
	     <kendo:grid-column title="EB&nbsp;Reading" field="initailReading"  filterable="true" width="80px" ></kendo:grid-column>
		 
		 <kendo:grid-column title="DG&nbsp;Reading" field="dgReading"  filterable="true" width="80px" ></kendo:grid-column>
			...
			<kendo:dataSource-schema-model id="prePaidId">
				<kendo:dataSource-schema-model-fields>
					...
					<kendo:dataSource-schema-model-field name="meterNumber" type="string">
					 <kendo:dataSource-schema-model-field-validation required="required"/>
  					...
  					<kendo:dataSource-schema-model-field name="initailReading" type="number"/>
  					
  					<kendo:dataSource-schema-model-field name="dgReading" type="number" defaultValue="0"/>
  					
  					<kendo:dataSource-schema-model-field name="readingDate" type="date" defaultValue="0"/>
...
<%-- <div id="meterWizardPopUp" hidden="true" style="overflow: hidden; height: 400px;">
					...
					<div class="tab-content" >
							<table>
									...
									<td>
										<div class="control-group">
											...
											<div class="controls">
												...
												<select id="propertyId" name="propertyId" data-required-msg="Select Property" required="required"></select>
											</div>
										</div>
									</td>
								</tr>
								<tr>
									<td>
										<div class="control-group">
											...
											<div class="controls">
												...
												<select id="personId" name="personId" data-required-msg="Select Person" onchange="accountNumberChange()" required="required"></select>
											</div>
										</div>
									</td>
									</tr>
									...
									<td>
										<div class="control-group">
											<label class="control-label" for="ca_no">CA&nbsp;Number&nbsp;*</label>
											<div class="controls">
												<form:input type="text" id="ca_no"  path="ca_no" cssStyle="width: 159px;" onchange="checkmtr()" />
											</div>
										</div>
									</td>
								</tr>
								<tr>
									<td>
										<div class="control-group">
											<label class="control-label" for="meterNumber">Meter&nbsp;Number&nbsp;*</label>
											<div class="controls">
								...
								<tr>
									<td>
										<div class="control-group">
											...
											<div class="controls">
												<form:input type="number" id="balance" path="previousBalance" cssStyle="width: 175px; height: 28px;"/>
											</div>
										</div>
									</td>
								</tr>
								<tr>
									<td>
										<div class="control-group">
											...
											<div class="controls">
												<form:input type="text" id="readingDate" path="serviceStartDate" cssStyle="width: 176px; height: 30px; " />
											</div>
										</div>
									</td>
								</tr>		
...
<!-- 												<select id="personId" name="personId" data-required-msg="Select Person" onchange="accountNumberChange()"></select>
 -->											</div>
						    ...
						    <ul class="pager wizard" style="margin-top: 5px;">
							<li><a style="background-color: rgb(236, 236, 236);"><input type="button" value="Submit" id="saveMeterData"></a></li>
							<li><a style="background-color: rgb(236, 236, 236);"><input type="button" value="Close" onclick="return closePop()"></a></li>
							</ul>
...
/* -----------------------------------------------For saving Old meter data  (Vijju) ----------------------------------------- */
...
function meterHistoryData(){
	...
	var ebReading = selectedItem.initailReading;
...
function checkmtr(){
	...
	var ca_Number=$("#ca_no").val();
	//alert(ca_Number);
	   if(ca_Number==''){
		   alert("CA Number Required");
		   return false;
	   }
	
	//alert($("#personId").val());
	  $.ajax({
		   url : "./meterGeneration/checkCANumbAvailability/"+ca_Number,
		   type: "GET",
		   dataType:"text",
		   success : function(response)
		   {	//alert(response);
			  if(response == 0){
			         alert("CA Number Already Assigned for Other Property");
					 $("#ca_no").val("");
				 return false;
			  }else{
				  return true;
...
$("#saveMeterData").on("click", function(e){
	...
	var initailReading = $("#initailReading").val();
	...
	var balance=$("#balance").val();
	  ...
	  if(blockName==null){
		 ...
		 // alert("hhh");
		   $.ajax({
		   url : "./meterGeneration/meterNumberAvailability123/"+meterNumber,
		   type: "GET",
		   dataType:"text",
		   async: false,
		   success : function(response)
		   {	
			   if(response == "Meter Already Assigned"){
				   ...
				   $.ajax({
					   url : "./meterGenerationCreate?personId="+personId+"&propertyId="+propertyId+"&blockName="+blockName+"&meterNumber="+meterNumber+"&ca_Number="+ca_Number+"&readingDate="+readingDate+"&initailReading="+initailReading+"&dgReading="+dgReading+"&balance="+balance,
					   type: "GET",
...
$("#grid").on("click", ".k-grid-meterGrid", function(e) {
	...
	$("#readingDate").val("");
		  ...
		  function accountNumberChange(){
				...
				var propertyId=$("#propertyId").val();
				$.ajax({
					   url : "./meterGeneration/accountNumber?personId="+personId+"&propertyId="+propertyId,
					   type: "GET",
					   contentType: "application/json; charset=utf-8",
					   dataType:"json",
					   success : function(response)
...
function onChangeBillsList(arg) {
	...
	caNum=selectedItem.ca_no;
	ebRead=selectedItem.initailReading;
	dgRead=selectedItem.dgReading;
						...
						kendo.ui.validator,
						{
							rules : { // custom rules          	
								...
								contactContentValidation : function(input,
									...
									if (input
											.filter("[name='meterNumber']").length
											&& input.val()) {
							...
							CANumberValidation : function(input,
									params) {
								//check for the name attribute 
								if (input
										.filter("[name='ca_no']").length
										&& input.val()) {
									contactContents = input.val();
									$
											.each(
							...
							CANumbertUniqueness : function(input) {
								if (input
										.filter("[name='ca_no']").length
										&& input.val() && flag1 != "") {
									flag1 = "";
							 ...
							 meterNumberValidation : function(input, params) {
									if (input.attr("name") == "meterNumber") {
										return $.trim(input.val()) !== "";
									}
									return true;

								},
							 caNumberValidation : function(input, params) {
										if (input.attr("name") == "ca_no") {
											return $.trim(input.val()) !== "";
										}
										return true;
							...
							messages : {
								...
								CANumbertUniqueness : "CA Number already exists",
								meterNumberValidation:"Meter Number Requierd",
								caNumberValidation :"CA Number Required"
							}
						});
