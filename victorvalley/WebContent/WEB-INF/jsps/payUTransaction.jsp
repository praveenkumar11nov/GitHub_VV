...
<script type="text/javascript"
	...
	<%-- <c:url value="/paytmTransaction/billRead" var="readUrl" /> --%>
	...
	<kendo:grid name="grid" edit="serviceParameterMasterEdit" pageable="true" scrollable="true" sortable="true" reorderable="true" groupable="true" selectable="true"
		...
		<kendo:grid-toolbarItem name="exportTxnDataDatasToExcelForPayU" text="Export To Excel" ></kendo:grid-toolbarItem>		
  	    <kendo:grid-toolbarItem name="exportTxnDataDatasToPDFForPayU" text="Export To PDF" ></kendo:grid-toolbarItem>			
		...
		<kendo:grid-columns>
			 ...
			 <kendo:grid-column title="Transaction&nbsp;Date" field="transactionDate" width="130px" format="{0:dd/MM/yyyy}"/>
			...
			<kendo:grid-column title="PayU Money Id&nbsp;*" field="payumoneyId" width="130px" filterable="true">
			<kendo:grid-column-filterable>
				<kendo:grid-column-filterable-ui>
					<script>
						function ledgerTypeFilter(element) {
			...
			<kendo:grid-column title="Transaction Id&nbsp;*" field="TransactionId" width="130px" filterable="true">
			<kendo:grid-column-filterable>
				<kendo:grid-column-filterable-ui>
					<script>
						function ledgerTypeFilter(element) {
		...
		<%-- 	<kendo:grid-column-filterable>
			...
			<kendo:grid-column title="Order&nbsp;Id*" field="orderId" width="100px" filterable="true"/>
			<kendo:grid-column title="Failed&nbsp;Reason" field="failedReason" width="100px" filterable="true"/>
...
/* 	var monthType="monthType";
	...
	function searchByMonth() {
	   ...
	   if(splitmonth[1]==splitmonth1[1])
		...
		$.ajax({
			type : "POST",
			url : "./payUTransactionReadUrl/byMonth/"+month+"/"+month1,
			dataType : "json",
	...
	/* ***************************************Export to Excel******************************************** */
	 ...
	 $("#grid").on("click",".k-grid-exportTxnDataDatasToExcelForPayU",function(e){
		 ...
		 window.open("./onlineTransaction/exportonlineTransactionDataToExxcelForPayU/"+month+"/"+month1);
	 });
	 ...
	 $("#grid").on("click",".k-grid-exportTxnDataDatasToPDFForPayU",function(e){
		 ...
		 window.open("./onlineTransaction/exportonlineTransactionDataToPDFForPayU/"+month+"/"+month1);
	 });
</script>
